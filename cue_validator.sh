...
if [ "$(echo " WAVE MP3 AIFF " | grep -o " $(grep -m1 FILE "$1" | sed -n 's/.* //p' | tr -d '\f\r') ")" ]; then
  #~  General parsing
  ...
  for p in $(grep -o "TRACK\|INDEX 01" "$1" | tr "\n" " " | sed 's/INDEX 01 /&\n/g'); do
    [ "$p" = "TRACK INDEX 01 " ] || echo "Track w/o matching index"
  done
  for p in $(grep -o "TRACK\|PREGAP\|INDEX 00" "$1" | tr "\n" " " | sed 's/TRACK/\n&/g'); do
    [ "$p" = "TRACK PREGAP INDEX 00 " ] && echo "PREGAP and INDEX 00 on same track"
  done
